...
func TestVerifyNamesTheKeyFileItCouldNotRead(t *testing.T) {
	err := runVerify(context.Background(), &bytes.Buffer{},
		"registry.example/demo/reviewer:1.0.0", filepath.Join(t.TempDir(), "absent.pub"), registryOptions{})
...
func TestVerifyNamesAKeyFileThatIsNotAKey(t *testing.T) {
	...
	err := runVerify(context.Background(), &bytes.Buffer{},
		"registry.example/demo/reviewer:1.0.0", path, registryOptions{})
...
func TestSignParsesAReferenceWhoseHostCarriesAPort(t *testing.T) {
	repo, tag, err := newRepository("127.0.0.1:45100/demo/agent-skills/reviewer:1.0.0", registryOptions{plainHTTP: true})
...
func TestSignNamesTheKeyFileItCouldNotRead(t *testing.T) {
	err := runSign(context.Background(), &bytes.Buffer{},
		"127.0.0.1:45100/demo/reviewer:1.0.0", filepath.Join(t.TempDir(), "absent.key"), registryOptions{plainHTTP: true})
