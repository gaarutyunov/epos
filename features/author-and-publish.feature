Feature: Author and publish a skill
  ...
  and keeps it in a local store, and pulls published skills back out of
  whichever registry you already run.

  Publishing is not part of Epos for now: the write path would have to redirect
  the upload session to upstream, and oras-go refuses a cross-host upload
  Location as the fix for GHSA-jxpm-75mh-9fp7. Skills are published to the
  registry directly, with whatever client already holds your credentials.
  ...
  Scenario: A published skill is pulled into an empty store
    ...
    And it is published to the registry
