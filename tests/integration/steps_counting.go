...
// publishesFor totals epos.publishes for a repository, read out of the
// exporter output the same way downloads are.
func publishesFor(out *metricsOutput, repository string) (int64, error) {
		...
		for _, sm := range payload.ScopeMetrics {
			for _, m := range sm.Metrics {
				...
				for _, dp := range m.Data.DataPoints {
					...
					for _, a := range dp.Attributes {
						if a.Key == "repository" {
							repo, _ = a.Value.Value.(string)
						}
						fmt.Fprintf(&key, "%s=%v;", a.Key, a.Value.Value)
					}
					if repo == repository {
						latest[key.String()] = dp.Value
					}
				}
			}
		}
